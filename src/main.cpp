...
int main(int /*argc*/, char* /*argv*/[]) {
    ...
    try {
        ...
        Logger::critical("Failed to set resource directory");
        Logger::exception(e);
        exit(EXIT_FAILURE);
    }
    ...
    if (SDL_Init(SDL_INIT_VIDEO) < 0) {  // also inits SDL_INIT_EVENTS
        Logger::critical("SDL's video subsystem failed to initialize\nSDL Error: " + std::string(SDL_GetError()));
        exit(EXIT_FAILURE);
    }
    if (SDL_Init(SDL_INIT_AUDIO) < 0) {
        Logger::critical("SDL's audio subsystem failed to initialize\nSDL Error: " + std::string(SDL_GetError()));
        exit(EXIT_FAILURE);
    }
    if (TTF_Init() != 0) {
        Logger::critical("SDL's TTF rendering engine failed to initialize\nTTF error: " + std::string(TTF_GetError()));
        exit(EXIT_FAILURE);
    }

    // init program; scope to force deletion before SDL clean-up
