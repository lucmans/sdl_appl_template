...
constexpr void fit_center(SDL_Rect& src, const SDL_Rect& dst, const int padding = 0) {
    const double src_ar = (double)src.w / (double)src.h,
                 dst_ar = (double)dst.w / (double)dst.h;

    // case 1: dst is too narrow/long for src
    if (src_ar > dst_ar) {
        src.w = dst.w - (padding * 2);
        src.h = (int)std::round((double)src.w / src_ar);
        src.x = dst.x + padding;
        src.y = dst.y + padding + (int)std::round((double)src.h / 2.0);
    }
    // case 2: dst is too wide/short for src
    else if (src_ar < dst_ar) {
        src.h = dst.h - (padding * 2);
        src.w = (int)std::round((double)src.h * src_ar);
        src.x = dst.x + padding + (int)std::round((double)src.w / 2.0);
        src.y = dst.y + padding;
    }
